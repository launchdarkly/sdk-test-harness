					...
					t.Run(h.IfElse(trailingSlash, "base URI has a trailing slash",
						...
						for _, method := range c.availableFlagRequestMethods() {
							t.Run(string(method), func(t *ldtest.T) {
								...
								streamURI := strings.TrimSuffix(dataSystem.Synchronizers.primary.Endpoint().BaseURL(), "/")
								if trailingSlash {
									streamURI += "/"
								}
...
func (c CommonStreamingTests) RequestViaHTTPProxy(t *ldtest.T) {
	...
	t.Run("http proxy", func(t *ldtest.T) {
		...
		streamURI := strings.Replace(dataSystem.Synchronizers.primary.Endpoint().BaseURL(), "localhost", "not.valid.local", 1)

		u, err := url.Parse(dataSystem.Synchronizers.primary.Endpoint().BaseURL())
		if err != nil {
