		...
		t.Run("single-kind anonymous context redacts all attributes", func(t *ldtest.T) {
			...
			for _, valueType := range getValueTypesToTest(t) {
				t.Run(testDescFromType(valueType), func(t *ldtest.T) {
					...
					m.In(t).Assert(payload, m.ItemsInAnyOrder(
						IsIndexEventForContext(context),
						IsValidFeatureEventWithConditions(t, false, context, m.JSONProperty("context").Should(matcher)),
						EventHasKind("summary"),
					))
				})
			}
		...
		t.Run("multi-kind with anonymous context redacts attributes appropriately", func(t *ldtest.T) {
			...
			for _, valueType := range getValueTypesToTest(t) {
				t.Run(testDescFromType(valueType), func(t *ldtest.T) {
					...
					payload := events.ExpectAnalyticsEvents(t, defaultEventTimeout)
					m.In(t).Assert(payload, m.ItemsInAnyOrder(
						IsIndexEventForContext(multiContext),
...
// CSPE 1.2.1: prerequisite events emitted equivalent to direct evaluation (server-side)
// CSPE 1.2.3: prerequisite events emitted before dependent flag's event
// CSPE 1.2.4: recursive prerequisites generate events (flag1→flag2→flag3)
