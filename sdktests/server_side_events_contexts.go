...
func doServerSideEventContextTests(t *ldtest.T) {
	...
	flags := data.FlagFactoryForValueTypes{
		KeyPrefix:    "ServerSideEvalEventContextFlag",
		ValueFactory: data.SingleValueFactory(flagValue),
	}
	debugFlags := data.FlagFactoryForValueTypes{
		KeyPrefix:      "ServerSideEvalEventContextDebugFlag",
		ValueFactory:   data.SingleValueFactory(flagValue),
		BuilderActions: func(b *ldbuilders.FlagBuilder) { b.DebugEventsUntilDate(ldtime.UnixMillisNow() + 100000) },
	}
	flag := flags.ForType(servicedef.ValueTypeAny)
	debugFlag := debugFlags.ForType(servicedef.ValueTypeAny)
