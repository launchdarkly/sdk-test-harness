...
// doCollectionCardinalityTests runs with no data source, so everything the SDK
// serves comes out of the store, and the cache is off so every read hits the
// store.
//
...
//
// Store mode governs the write path, and with no data source there is nothing to
// write, so read mode is the only mode these cases need.
func (s *ServerSidePersistentTests) doCollectionCardinalityTests(t *ldtest.T) {
	...
	s.runWithEmptyStore(t, "collection holds one flag", func(t *ldtest.T) {
		...
		client := NewSDKClient(t, s.uncachedReadModePersistence())
...
// uncachedReadModePersistence configures a read-only store with no cache. The
// caller passes it as the only configurer, so the store is the only source of
// data and every read reaches it.
func (s *ServerSidePersistentTests) uncachedReadModePersistence() *Persistence {
	persistence := NewPersistence()
	persistence.SetStore(servicedef.SDKConfigPersistentStore{
		Type: s.persistentStore.Type(),
		DSN:  s.persistentStore.DSN(),
	})
	persistence.SetStoreMode(servicedef.DataStoreModeRead)
