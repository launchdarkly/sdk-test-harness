...
// This file covers the client-side consumer half of bootstrap: an application hands the
// SDK pre-fetched flag data at start() time, and the SDK must apply it to its flag store
// before making any network calls. The payload is the serialized form of a server-side
// SDK's allFlagsState result.
//
// bootstrapFlagMetadata is one entry in a bootstrap payload's "$flagsState" object. Each
// field is emitted only when it is set, so a zero-valued bootstrapFlagMetadata renders as
...
// toValue renders the metadata entry as a JSON object. Unset fields are omitted, and the
// boolean fields are omitted when false, to match how a server-side SDK serializes its
// allFlagsState result.
...
// server-side SDK's allFlagsState result produces: a flat map of flag key to variation
// value, plus the reserved "$flagsState" and "$valid" keys.
...
// Flag adds a top-level flag value and its "$flagsState" entry. A nil metadata renders as
...
// Valid sets "$valid" explicitly. Leave it unset to omit the property; the SDK must treat
// an absent "$valid" as true.
...
// payload shape that older server-side SDKs emit.
...
// network calls for flag data. We must send an explicit connection mode rather than omit
// dataSystem entirely: validateSDKConfig (testapi_sdk_client.go) rejects a config with
// neither dataSystem nor persistentDataStore.
...
// and no synchronizer, so the only possible source for a non-default value is the
// bootstrap object.
...
// doClientSideBootstrapSatisfiesInitializationTest verifies that valid bootstrap data
// alone satisfies the SDK's initialization condition, without waiting for a network
// response.
//
// The data system here is live, not offline: it has a single connection mode with only a
// streaming synchronizer (no polling initializer), which accepts the connection but never
// sends any data, via mockld.BlockingUnavailableSDKData. initCanFail is false and
...
// doClientSideBootstrapMetadataInEventsTest verifies that the SDK merges the
// "$flagsState" metadata (variation, version, trackEvents, trackReason, reason) into its
// internal flag representation. debugEventsUntilDate is not exercised here; debug events
// are out of scope for this test.
//
// Two flags are used because a single flag with both "trackEvents" and "trackReason" set
// cannot distinguish which of the two is actually driving the feature event: a feature
// event with a reason is emitted whenever "trackReason" is true, regardless of
// "trackEvents". The reasonFlag below covers trackReason plus the reason payload; the
// noReasonFlag covers trackEvents in isolation (with trackReason false and no reason),
// proving trackEvents on its own is what the SDK honored.
//
// The observable proof is the analytics events produced by later evaluations, not the
// all-flags result: a client-side SDK's AllFlags returns flag values only, by design, so
// this test also asserts that the metadata does not leak into that result.
...
// failed on the producing SDK: it still appears at the top level with a null value and
// still has a "$flagsState" entry with no "variation" field. A null flag value means
...
// doClientSideBootstrapInvalidPayloadTest verifies that with "$valid": false the SDK
// still ingests and uses whatever flag data is present in the payload, and does not throw
...
// doClientSideBootstrapLegacyPayloadTest verifies that when "$flagsState" is absent, as
// in payloads from older server-side SDKs, the SDK still ingests the flag values. The
// payload also omits "$valid", which the SDK must treat as true.
...
// doClientSideBootstrapReplacedByLiveDataTest verifies that bootstrap data does not
// persist past the first successful network update.
//
// The two halves are separate subtests with separate SDK clients on purpose. The mock
// services run in the harness process, so a live synchronizer's first payload can land
// before the harness issues its first evaluate() command (see the comment on
// validatePayloadReceived in common_tests_stream_fdv2.go). Asserting the pre-network
// bootstrap value on a client that also has a live endpoint would be a race.
func doClientSideBootstrapReplacedByLiveDataTest(t *ldtest.T) {
	...
	// liveData. The live synchronizer's first payload must replace bootstrap data, not merge
	// with it; if the SDK merged instead of replaced, this flag would still evaluate to
